...
void check(ll i, vll &arr,ll x,ll &ans){
    if(i == x){
        return;
    }
    ans++;
    check(arr[i]-1,arr,x,ans);
}
void check2(ll i, vll &arr,ll x,ll &ans, vll &fin){
    if(i == x){
        return;
    }
    fin[i] = ans;
    check2(arr[i]-1,arr,x,ans,fin);
}
...
void LetsSolveIt(){
    ...
    for(ll i=0;i<n;i++){
        ...
        if(fin[i] == 0){
            val = i;
            check(arr[i]-1,arr,val,ans);
            ans++;
            check2(arr[i]-1,arr,val,ans,fin);
            fin[i] = ans;
        }
    }
    for(ll i=0;i<n;i++)
