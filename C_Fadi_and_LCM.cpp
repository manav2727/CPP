...
ll ans = inf;
vll all_subset_sums;
void findAns(vll &arr, ll l, ll r,ll x,ll prod){
    if(l>r){
        ans = min(ans,max(prod,x/prod));
        // all_subset_sums.push_back(prod);
        return;
    }
    findAns(arr, l+1, r, x, prod);
    findAns(arr, l+1, r, x, prod*arr[l]);
}
...
void LetsSolveIt(){
    ...
    //     temp *= v[i];
    //     ans = min(ans,max(x/temp,temp));
    // }
    findAns(v, 0, v.size()-1, x,1ll);
    cout<<x/ans<<" "<<ans<<"\n";
}
