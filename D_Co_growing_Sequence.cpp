...
void LetsSolveIt(){
    ...
    cout<<0<<" ";
    for(ll i=1;i<n;i++){
        ll temp = arr[i]^prev; // To cut off all the 0s and 1s, which already satisy after ANDing
        cout<<(temp&prev)<<" "; // To cut off all the extra 1s which came from current, and which were not necessary(0 in prev)
        // as after AND operation in final, anaything gets 0 after AND with 0
        prev = arr[i]^(temp&prev);
    }
    cout<<"\n";
}
// prev - 1011, curr - 1101
// after XORing both, 0th and 3rd index 1s already satisfy, we want 1 at 2nd index in curr
// after xor - 0110, now we only want 0010 to put 1 at 2nd index in curr
// so to make 0110 -> 0010, AND this 0110 with prev, so only the required 1s will remain, and others which came from curr, will be cut off
