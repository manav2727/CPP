...
// const ll N = 1e5+1;
void traverse(ll left, ll right,vll const &arr,vll &trv,ll dept=0)
{
    if(right<left)
        return;
    if(left == right)
    ...
    ll maxi = left;
    for(ll i=left+1;i<=right;i++) // traverse from left+1 to right, considered left indexed as the max initially
    {
        if(arr[maxi]<arr[i])
            maxi = i;
    }
    trv[maxi] = dept;
    traverse(left, maxi - 1, arr, trv, dept+1); //Increasing depth every time we traverse left or right from maximum wherever we get it
    traverse(maxi+1, right, arr, trv, dept+1);
}
