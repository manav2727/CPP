...
void LetsSolveIt(){
    var(n)var(g)var(b)
    ll miniG = cl(n,2);
    ll noOfBlocks = miniG/g;
    ll lenOfBlocks = noOfBlocks * (g + b);
    if(miniG % g == 0)
        lenOfBlocks -= b;
    else 
        lenOfBlocks += (miniG % g);
    cout<<max(n, lenOfBlocks)<<"\n";
    // 9 2 1
    // we want atleast 5 g, so we can take - 2g 1b  2g 1b  1g
    // miniG = 5, noOfBlocks = 5/2 = 2, lenOfBlocks = 2*3 = 6
    // but we want one more 1g .... lenOfBlocks += 5 % 2 = 7
    // 8 2 1 - 2g 1b 2g
    // miniG = 4, noOfBlocks = 4/2 = 2, lenOfBlocks = 2*3 = 6
    // we wont require extra 1b (miniG % g == 0), so we remove that extra b
}
