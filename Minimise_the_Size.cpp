...
void LetsSolveIt(ll n){
    ...
    string s1 = bin(n),s="";
    ll temp = 0;
    while(temp<64 && s1[temp] == '0')
        temp++;
    for(ll i=temp;i<64;i++){
        s += s1[i];
    }
    vll v;
    ll len = s.length();
...
// 		}
// 		printf("%d\n",ans.size());
// 		for(int i=0;i<ans.size();i++)printf("%ld%c",ans[i],i+1==ans.size()?10:32);
// 	}
// }