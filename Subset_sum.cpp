...
ll tab[6][13],calls=0,calls2=1;
vll all_subset_sums;

ll subsetSum(vll &arr, ll n,ll sum1){
    calls++;
    if(sum1 == 0)
        return 1; // condition satisfied
    
    if(n<=0)
        return 0; // no elements left in array to check further
    
    if(tab[n-1][sum1] != 0)
        return tab[n-1][sum1];
    else{
        if(arr[n-1] <= sum1){ // n-1 because it is 0 based indexing
            // Here we do two calls because we
            // don't know which value is
            // full-fill our criteria
            // that's why we doing two calls
            return tab[n-1][sum1] = subsetSum(arr, n-1, sum1 - arr[n-1]) // taking that value
                                            || subsetSum(arr, n-1, sum1); // not taking that value
        }
        else{
            return tab[n-1][sum1] = subsetSum(arr, n-1, sum1); // no option, we dont take it.
        }
    }
}
