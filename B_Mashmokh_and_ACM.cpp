...
ll dp[2002][2002];
ll maxi(ll n, ll k, ll i){
    if(dp[i][k] != -1)
        return dp[i][k]; // from the below's step, 1st, all the last columns count will be 1, and after that that, 2nd last columns data, then 3rd last, ....
    if(k == 0)
        return 1; // whenever k goes beyond last element, count++
    ll count = 0;
    for(ll j = i;j <= n;j += i){
        count = (count + maxi(n, k-1, j)%mod)%mod;
    }
    return dp[i][k] = count;
}
