...
void solve(){
    ...
    for(ll i=1;i<=n;i++)
    {
        ...
        else
        {
            ...
            {
                col.push_back(occ[i][j]);
            }
        }
    }
    // for(ll i=0;i<col.size();i++)
    //     cout<<col[i]<<" ";
    for(ll i=0;i<=col.size()-k;i+=k)  // co.size() - k because, for those numbers, it cannot be equally divided into k colors, so atleast k remaining(which were less than k)
    {
        for(ll j=0;j<k;j++)
        {
            ans[col[i+j]] = j+1; // Traversing every k elements of remaining element's, and putting numbers from 1 to k
        }
    }
    for(ll i=0;i<n;i++)
        cout<<ans[i]<<" ";
