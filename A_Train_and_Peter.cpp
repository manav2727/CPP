...
void LetsSolveIt(){
    vars(s)
    vars(fst)vars(snd)
    ll slen = s.length(),fstlen = fst.length(),sndlen = snd.length();
    if(fstlen +sndlen > slen){
        cout<<"fantasy";
        return;
    }
    bool fwd=true,bck=true;
    size_t found1 = s.find(fst);
    if (found1 == string::npos)
        fwd=false;
    

    string check="";
    for(ll i=found1+fstlen;i<slen;i++){
        check += s[i];
    }

    size_t found2 = check.find(snd);
    ...
    size_t found3 = s.find(fst);
    if (found3 == string::npos)
        bck=false;
    
    string check2="";
    for(ll i=found3+fstlen;i<slen;i++){
        check2 += s[i];
    }

    size_t found4 = check2.find(snd);
