...
void solve(){
    ...
    ll ans = 0,sum1 = arr[0];
    for(ll i=1;i<n;i++){
        ans = max(ans, ll(ceil((double)(100ll*arr[i] - sum1*k)/k)));
        sum1 += arr[i];
    }
    cout<<ans<<"\n";

}
// If at any ith point, inflation > k/100, then noramlly we would add some x to i-1
// but then it is possible that (i-1)th inflation would be > k/100
// Therfore it is optimal to add x to 0th element
// So now,after adding x We know Inf of ith element = arr[i]/(sum till arr[i-1] + x)
// Also Inf of ith element <= k/100
// Therefore arr[i]/(sum till arr[i-1] + x) <= k/100
// 100*arr[0] <= (sum till arr[i-1])*k + x*k
// 100*arr[0] - (sum till arr[i-1])*k <= x*k
// x >= (100*arr[0] - (sum till arr[i-1])*k)/k
// Now we find max x, which can satisy every element
