...
void solve(){
    ...
    ll quest = 0,cnt2=0,posn=0,negn=0;
    for(auto &i:t){
        if(i == '+')
            cnt2++,posn++;
        else if(i == '-')
    ...
    ld powr = pow(2,quest); // Total combinations of quest places using 2 symbols -> + and -
    ld ncr = nCr(quest,pos-posn); // Out of total quest positions, in how many ways we can put required + in quest
    ld ans = ncr/powr; // Probablity -> (Required +)/(Total combinations of + and -) 
