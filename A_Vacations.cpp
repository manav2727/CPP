...
ll miniRest(vll &arr,ll n, ll i, ll last){
    if(i >= n)
    ...
    if(arr[i] == 0){ // Rest
        dp[i][last] = miniRest(arr, n, i+1, 0) + 1; // compulsory rest
    }
    else if(arr[i] == 1){
        if(last == 1){
            dp[i][last] = miniRest(arr, n, i+1, 0) + 1; // compulsory rest
            ...
            dp[i][last] = miniRest(arr, n, i+1, 1);
        }
    }
    else if(arr[i] == 2){
        if(last == 2){
            ...
            dp[i][last] = miniRest(arr, n, i+1, 2);
        }
    }
    else if(arr[i] == 3){
        if(last == 2){
            ...
            dp[i][last] = min(miniRest(arr, n, i+1, 1), miniRest(arr, n, i+1, 2));
        }
    }
    return dp[i][last];
}
