...
void printr(vll a, ll start, ll end)
{
...
void solve()
{
    ...
    for(int k=0;k<v.size()/2;k++)
    {
        ...
        {
            swap(v[i][k],v[k][v.size()-i-1]) ;  
        }
        for(int i=v.size()-1-k;i>=(1+k);i--)
        {
            swap(v[v.size()-1-k][i],v[i][k]) ;  
        }
        for(int i=1+k;i<v.size()-(1+k);i++)
        {
            swap(v[v.size()-(1+k)][i],v[v.size()-i-1][v.size()-(1+k)]) ;  
        }
    }
