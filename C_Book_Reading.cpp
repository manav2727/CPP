...
void solve(){
    ...
    for(ll i=0;i<=9;i++)
    {
        arr[i] = (i*m)%10; // Take sum of all till 10th multiple
        sum1 += arr[i];
    }
    n /= m; // Meke m equal parts 
    sum1 *= n/(10); // Finally it is nothing but n is divided by parts containing sum from 0 to m*10 in each part
    //Example: 100 5....100 divided into 50 50....and in that each 50 part, there contains sum of all divisors of 5 from 0 to 50
    for(ll i=0;i<n%10;i++)
        sum1 += arr[i+1];
    cout<<sum1<<"\n";
}
