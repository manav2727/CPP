...
void solve(){
    ...
    ll sum1 = (n*(n-1))/2,sum2 = (n*(n-1))/2,a=(n*(n-1))/2;
    //cout<<a<<"\n";
    for(ll i=0;i<n-1;i++)
    {
        ll cnt = arr1[i];
        ...
        {
            //cout<<arr[j]<<"\n";
            if(gcd(cnt,2*arr1[j])==1)
                sum1--;
        }
    }
   // for(ll i=0;i<n;i++)
       // cout<<arr1[i]<<" ";
   // cout<<max(sum1,max(sum2,a-((n-cnt)*(n-cnt-1))/2))<<"\n";
   //cout<<max(a-((cnt)*(cnt-1))/2,a-((n-cnt)*(n-cnt-1))/2)<<"\n";
   cout<<sum1<<"\n";
}
