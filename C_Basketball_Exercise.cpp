...
ll maxi(vll &arr,vll &arr2, ll n, ll last, ll i){
    if(i>=n)
        return 0;
    ...
    if(last == 0){
        return dp[i][last] = max(arr2[i] + maxi(arr,arr2,n,1,i+1), arr2[i] + maxi(arr,arr2,n,1,i+2));
    }
    
    return dp[i][last] = max(arr[i] + maxi(arr,arr2,n,0,i+1), arr[i] + maxi(arr,arr2,n,0,i+2));
...
void LetsSolveIt(){
    ...
    ll ans1 = maxi(arr,arr2,n,0,0),ans2 = maxi(arr,arr2,n,1,0);
