...
void LetsSolveIt(){
    ...
    for(ll i=0;i<n;i++){
        cin>>temp;
        freq[temp]++;
        cnt[freq[temp]]++; // do not remove count of prev freq, as we can take that freq of planks also
        // eg. if 2 3 4  is there of freq of suppose 7 8 9 resp, then we can take 2 2 4.
    }
    var(x)
    for(ll i=0;i<x;i++){
        varc(s)var(k)
        ...
        if(cnt[8]>=1 || (cnt[6]>=1 && cnt[2]>=2) || (cnt[4]>=2) || (cnt[4]>=1 && cnt[2]>=3)){
            // cout<<cnt[2]<<" "<<cnt[4]<<" "<<cnt[6]<<" "<<cnt[8]<<"\n";
            // cnt[2] >= 2, becoz, 1 cnt of 2 freq will be of cnt[4] also
            // similarly while counting freq of cnt[4], there will be 1 cnt[2] extra, so check min 3 of cnt[2]
            cout<<"YES\n";
        }
        else{
            cout<<"NO\n";
