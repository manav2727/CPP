...
void solve(){
    var(n)
    ll arr[n],sum1=0,tcnt=0,cnt=0,thcnt=0,ocnt=0;
    for(ll i=0;i<n;i++)
    {
        cin>>arr[i];
        if(arr[i]==2)
    ...
    //if(ocnt%4==0)
    //{
 //       cout<<n-(tcnt/2)-ocnt+(ocnt)/4<<"\n";
    //}
    //else
       // cout<<n-(tcnt/2)-ocnt+((ocnt)/4)+1<<"\n";
   // cout<<thcnt<<tcnt<<ocnt<<"\n";
    cout<<cnt+thcnt+(tcnt*2+ max((ll)0,ocnt)+3)/4<<"\n";
