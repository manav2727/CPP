...
void LetsSolveIt(){
    ...
    ll ans = min(zero,one),pref0 = 0,pref1 = 0;
    for(ll i=0;i<n;i++){
        if(s[i] == '0'){
            pref0++;
            zero--;
        ...
        // making 00001111
        ans = min(ans, pref1 + zero); // till current position, convert all prefix 1s to 0 
        //and after that position, all remaining 0s should be coverted to 1

        // making 11110000
        ans = min(ans, pref0 + one); // till current position, convert all prefix 0s to 1 
        //and after that position, all remaining 1s should be coverted to 0
    }
